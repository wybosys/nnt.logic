...
// 为了解决include顺序问题，和models里面的status保持一致
const STATUS_OK = 0;
const STATUS_PARAMETER_NOT_MATCH = -995;

// 检查模型和输入数据的匹配情况，简化模式（不处理第二级）
export function CheckInputStatus(proto: any, params: any): number {
    ...
    for (let key in fps) {
        ...
        if (fp.optional) {
            if (fp.valid && !IsEmpty(inp)) {
                let v = DecodeValue(fp, inp);
                if (!fp.valid(v))
                    return fp.valid.status != null ? fp.valid.status : STATUS_PARAMETER_NOT_MATCH;
            }
            continue;
        }
        if (IsEmpty(inp))
            return STATUS_PARAMETER_NOT_MATCH;
        // 判断是否合规
        if (fp.valid) {
            // 需要提前转换一下类型
            let v = DecodeValue(fp, inp);
...
export function CheckInput(proto: any, params: any): boolean {
    let sta = CheckInputStatus(proto, params);
