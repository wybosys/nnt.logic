...
export class RAudioStore implements IRouter {
    ...
    protected doUploadBinary(srv: IMediaStore, buffer: Buffer, type: string, pat: string[]): Promise<AudioUploadResult> {
    ...
    protected doUploadFile(srv: IMediaStore, file: IndexedObject): Promise<AudioUploadResult> {
        return new Promise(resolve => {
            let srcf = file.path;
    ...
    protected doConvertUpload(srv: IMediaStore, typ: AudioSupport, result: AudioUploadResult): Promise<AudioUploadResult> {
        return new Promise(resolve => {
    ...
    protected doDownload(srv: IMediaStore, src: string, pat: string[]): Promise<string> {
        return new Promise(resolve => {
            // 按照日期格式保存到目录中
            let today = new Date();
            let dir = this.hash.encode(today.getFullYear(), today.getMonth(), today.getDay());
