...
export class GetMedia {
    ...
    @enumerate(1, MediaSupport, [input, optional], "文件类型")
    type?: number;
...
const PAT_MEDIASTOREPATH = /^[0-9A-Za-z-+]+\/[0-9A-Za-z-+\.]+$/;

function IsMediaStorePath(path: string): boolean {
    return path.match(PAT_MEDIASTOREPATH) != null;
}
...
class RMediaStore implements IRouter {
    action = "mediastore";
    hash = new Hashids("mediastore");

    constructor() {
        this.supports[MediaSupport.MP3] = [
            Mime.Type('mp3')
        ];
        this.supports[MediaSupport.SPX] = [
            Mime.Type('spx')
        ];
        this.supports[MediaSupport.AMR] = [
            Mime.Type('amr'),
    ...
    async upload(trans: Transaction) {
        ...
        let m: UploadMedia = trans.model;
        let srv = static_cast<MediaStore>(trans.server);
        if (m.type == null)
            m.type = MediaSupport.MP3;
    ...
    protected doUploadBinary(srv: MediaStore, buffer: Buffer, type: string, pat: string[]): Promise<MediaUploadResult> {
    ...
    protected doUploadFile(srv: MediaStore, file: IndexedObject): Promise<MediaUploadResult> {
        return new Promise(resolve => {
            let srcf = file.path;
    ...
    protected doConvertUpload(srv: MediaStore, typ: MediaSupport, result: MediaUploadResult): Promise<MediaUploadResult> {
        return new Promise(resolve => {
    ...
    download(trans: Transaction) {
        ...
        let srv = static_cast<MediaStore>(trans.server);
        let m: DownloadMedia = trans.model;
        if (m.type == null)
            m.type = MediaSupport.MP3;
        let pat = this.supports[m.type];
        if (!pat) {
    ...
    protected doDownload(srv: MediaStore, src: string, pat: string[]): Promise<string> {
        return new Promise(resolve => {
            // 按照日期格式保存到目录中
            let today = new Date();
            let dir = this.hash.encode(today.getFullYear(), today.getMonth(), today.getDay());
...
export class MediaStore extends Rest {
    ...
    constructor() {
        super();
        this.routers.register(new RMediaStore());
    }
