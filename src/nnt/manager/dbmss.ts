...
// 获得一组数据
export async function Query<T>(clz: TransactionDef<T>, cmd: cmd_t, limit?: number, skip?: number): Promise<T[]> {
    let t = new Transaction<T, T[]>(clz);
    t.rdbproc = db => {
        let q: RdbCmdType;
    ...
    t.nosqlproc = db => {
        db.query(t.table, <Object>cmd, limit, skip, t, res => {
            if (res == null)
                t.resolve([]);
            else {
                let rcds = new Array();
...
export async function QueryOne<T>(clz: TransactionDef<T>, cmd: cmd_t): Promise<T> {
    ...
    t.nosqlproc = db => {
        db.query(t.table, <Object>cmd, 1, 0, t, res => {
            if (res == null || !res.length)
                t.resolve(null);
            else if (res.length > 1)
                t.resolve(null);
